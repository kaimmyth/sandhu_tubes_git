...
class Landcontroller extends Controller
{
  ...
  public function Get_Registration_details($id)
  {
    ...
    $toReturn['lease_details']= DB::table('leasedata')
      ...
      ->where('leasedata.id', $id)->select('leasedata.*', 'leasedata.id as Id','customers.*', 'customer_company.company_type as company_type' ,'customer_company.company as company_name' ,'lands.*',  'lands.id as LandID')->first();
    ...
    // if($toReturn['leasedata']->is_proprietor==1)
    // {
    //   $toReturn['ltdpvt'] = Customer::where('comp_id', $toReturn['leasedata']->cust_id)->where('is_proprietor',1)->get();
    // }
    // else
    // {
    // $toReturn['ltdpvt']=Customer::where('comp_id', $toReturn['leasedata']->cust_id)->where('is_proprietor',0)->orderBy('id','Desc')->get();
    // }
    // $data['directorData']=Customer::where('comp_id', $data['leasedata']->cust_id)->where('is_director',1)->where('is_proprietor',0)->get();
    // print_r( $toReturn['lease_details']);
    // exit;
    return $toReturn;
  }
  public function Get_Registration_history($land_id)
  {
      ...
      ->where('leasedata.land_id', $land_id)->select('leasedata.*', 'leasedata.id as Id', 'customers.*', 'customer_company.company as company_name','customer_company.id as company_id','lands.*', 'customers.id as custID', 'lands.id as LandID')->get()->toArray();
...
// return $results;
  ...
  public function Create_Transfer_Registration(Request $request)
  {
    ...
    $leaseData_add->based_rent = $leaseData->based_rent;
    $leaseData_add->insurance = $leaseData->insurance;
    $leaseData_add->maintanance = $leaseData->maintanance;
    $leaseData_add->additional_charge = $leaseData->additional_charge;
    $leaseData_add->net_payable = $leaseData->add_net_payable;
    $leaseData_add->tax = $leaseData->taxes;
    $leaseData_add->net_pay = $leaseData->netpaybill;
    $leaseData_add->lease_time = $leaseData->lease_time;
    $leaseData_add->lease_duration = $leaseData->lease_duration;
    $leaseData_add->start_date = $leaseData->start_date;
    ...
    $leaseData_add->transfer_amount = $request->transfer_amount;
  ...
  {
    $toReturn['customer']=Customer::where('id', $id)->first();
    // $data = Customer::where('id', $id)->first();
